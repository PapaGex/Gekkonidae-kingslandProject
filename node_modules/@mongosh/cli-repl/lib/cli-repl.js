...
class CliRepl {
    constructor(options) {
        ...
        this.logId = new service_provider_core_1.bson.ObjectId().toString();
        ...
        this.configDirectory = new config_directory_1.ConfigManager(this.shellHomeDirectory)
            .on('error', (err) => this.bus.emit('mongosh:error', err))
    ...
    async start(driverUri, driverOptions) {
        ...
        setup_logger_and_telemetry_1.default(this.logId, this.bus, () => pino_1.default({ name: 'mongosh' }, logStream), () => {
            var _a, _b, _c;
    ...
    async openLogStream() {
        ...
        try {
            const stream = fs_1.createWriteStream(this.logFilePath, { mode: 0o600 });
    ...
    async cleanupOldLogfiles() {
        ...
        for await (const dirent of dirHandle) {
            ...
            if (!id)
                continue;
            if (new service_provider_core_1.bson.ObjectId(id).generationTime < (Date.now() / 1000) - 30 * 86400) {
                try {
                    await fs_1.promises.unlink(path_1.default.join(dir, dirent.name));
                }
                catch (err) {
                    this.bus.emit('mongosh:error', err);
                }
    ...
    isPasswordMissing(driverOptions) {
    ...
    async requirePassword(driverUri, driverOptions) {
        ...
        try {
            try {
                driverOptions.auth.password = (await passwordPromise).toString();
            }
            finally {
                this.output.write('\n');
            }
